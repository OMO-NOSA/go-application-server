...
func NewGetInventoryRequest(server string) (*http.Request, error) {
	...
	operationPath := fmt.Sprintf("/visitor/inventory")
	if operationPath[0] == '/' {
		operationPath = "." + operationPath
	}
...
func NewPlaceOrderRequestWithBody(server string, contentType string, body io.Reader) (*http.Request, error) {
	...
	operationPath := fmt.Sprintf("/visitor/order")
	if operationPath[0] == '/' {
		operationPath = "." + operationPath
	}
...
func NewDeleteOrderRequest(server string, orderId int64) (*http.Request, error) {
	...
	operationPath := fmt.Sprintf("/visitor/order/%s", pathParam0)
	if operationPath[0] == '/' {
		operationPath = "." + operationPath
	}
...
func HandlerWithOptions(si ServerInterface, options ChiServerOptions) http.Handler {
	...
	r.Group(func(r chi.Router) {
		r.Get(options.BaseURL+"/visitor/inventory", wrapper.GetInventory)
	})
	r.Group(func(r chi.Router) {
		r.Post(options.BaseURL+"/visitor/order", wrapper.PlaceOrder)
	})
	r.Group(func(r chi.Router) {
		r.Delete(options.BaseURL+"/visitor/order/{orderId}", wrapper.DeleteOrder)
	})
	r.Group(func(r chi.Router) {
		r.Get(options.BaseURL+"/visitor/order/{orderId}", wrapper.GetOrderById)
	})
